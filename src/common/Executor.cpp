...
void Executor::reportRunSummary(ostream& str) const
{
  ...
  if ( in_state == RunParams::BadInput ) {
    ...
    str << "\nKernels(problem size , iterations/rep , loops/rep , reps)"
        << "\n-----------------------------\n";
    for (size_t ik = 0; ik < kernels.size(); ++ik) {
...
void Executor::outputRunData()
{
  ...
  filename = out_fprefix + "-per-loop-timing.csv";
  writeCSVReport(filename, CSVRepMode::PerLoopTiming, 3 /* prec */);
                              ...
                              size_t prec)
{
  ...
  if ( file ) {
    ...
    for (size_t iv = 0; iv < variant_ids.size(); ++iv) {
      varcol_width[iv] = max(prec+((mode == CSVRepMode::PerLoopTiming) ? 7: 2),
      ...
      for (size_t iv = 0; iv < variant_ids.size(); ++iv) {
        ...
        if ( (mode == CSVRepMode::Speedup) &&
          ...
          file << "Not run";
                     ...
                     mode == CSVRepMode::PerLoopTiming) &&
                    !kern->hasVariantToRun(vid) ) {
          file << "Not run";
        } else {
          if ((mode == CSVRepMode::PerLoopTiming) &&
              (kern->getNumLoops() == 0)) {
            file << "Num Loops Unknown";
          } else {
...
string Executor::getReportTitle(CSVRepMode mode)
{
  ...
  switch ( mode ) {
    ...
    case CSVRepMode::PerLoopTiming : {
      title = string("Mean Per Loop Runtime Report (usec.) ");
      break;
    }
                                         ...
                                         VariantID vid)
{
  ...
  switch ( mode ) {
    ...
    case CSVRepMode::PerLoopTiming : {
      if (kern->getNumLoops() > 0) {
        retval = kern->getTotTime(vid) / run_params.getNumPasses();
        retval /= kern->getRunReps();
        retval /= kern->getNumLoops();
        retval *= 1e6;
      } else {
        retval = 0.0;
      }
