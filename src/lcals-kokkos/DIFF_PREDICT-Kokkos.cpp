...
void diff_predict_helper(Index_type run_reps, Index_type ibegin,
                         Index_type iend, Index_type offset,
                         // Kokkos View
                         px_type &px,
                         // Kokkos View
                         cx_type &cx) {
  ...
  for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
    ...
    Kokkos::parallel_for(
        "diff_predict_helper_Kokkos Kokkos_Lambda",
...
void DIFF_PREDICT::runKokkosVariant(VariantID vid,
  ...
  auto diffpredict_lam = [=](Index_type i) { DIFF_PREDICT_BODY; };
  ...
  case Kokkos_Lambda: {
    ...
    diff_predict_helper(run_reps, ibegin, iend, offset, px_view, cx_view);
