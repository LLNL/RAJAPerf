...
template < size_t block_size >
void HALO_EXCHANGE_FUSED::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      HALO_exchange_fused_pack<block_size><<<pack_nblocks, pack_nthreads_per_block, shmem, res.get_stream()>>>(
      ...
      HALO_exchange_fused_unpack<block_size><<<unpack_nblocks, unpack_nthreads_per_block, shmem, res.get_stream()>>>(
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      for (Index_type l = 0; l < num_neighbors; ++l) {
        ...
        for (Index_type v = 0; v < num_vars; ++v) {
          Real_ptr var = vars[v];
          auto HALO_exchange_fused_pack_base_lam = [=] __device__ (Index_type i) {
                HALO_PACK_BODY;
              };
          pool_pack.enqueue(
      ...
      for (Index_type l = 0; l < num_neighbors; ++l) {
        ...
        for (Index_type v = 0; v < num_vars; ++v) {
          Real_ptr var = vars[v];
          auto HALO_exchange_fused_unpack_base_lam = [=] __device__ (Index_type i) {
                HALO_UNPACK_BODY;
              };
          pool_unpack.enqueue(
              RAJA::TypedRangeSegment<Index_type>(0, len),
...
RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(HALO_EXCHANGE_FUSED, Cuda)
