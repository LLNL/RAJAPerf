...
template < size_t block_size >
void HALOPACKING::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    stopTimer();
     ...
     getCout() << "\n HALOPACKING : Unknown Cuda variant id = " << vid << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(HALOPACKING, Cuda)
