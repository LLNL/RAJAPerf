...
#if defined(RAJA_ENABLE_TARGET_OPENMP) && defined(RAJA_ENABLE_TARGET_OPENMP)
...
#define MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_COPY_PACK_OMP_TARGET \
  initOpenMPDeviceData(pack_ptrs, h_pack_ptrs, 4 * num_neighbors * num_vars);

#define MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_COPY_UNPACK_OMP_TARGET \
  initOpenMPDeviceData(unpack_ptrs, h_unpack_ptrs, 4 * num_neighbors * num_vars);

#define MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_TEARDOWN_OMP_TARGET \
...
void MPI_HALOEXCHANGE_FUSED::runOpenMPTargetVariant(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx))
{
  ...
  if ( vid == Base_OpenMPTarget ) {
    ...
    MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_SETUP_OMP_TARGET;
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_COPY_PACK_OMP_TARGET;
      ...
      MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_COPY_UNPACK_OMP_TARGET;
    ...
    MPI_HALOEXCHANGE_FUSED_MANUAL_FUSER_TEARDOWN_OMP_TARGET;
    ...
    stopTimer();
     ...
     getCout() << "\n MPI_HALOEXCHANGE_FUSED : Unknown OMP Target variant id = " << vid << std::endl;
  }
}
