      ...
      *pi = m_pi_init;
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(Index_type i) {
			...
			// addition, multiplication, division, etc. All of these atomic operations are architecture dependent.
			// Atomics are advantageous from a correctness point of view
            Kokkos::atomic_add(&pi_view(0), dx / (1.0 + x * x));
          });
      // Moving the data on the device (held in the KokkosView) BACK to the pointer, pi.
