...
template < size_t block_size, size_t replication >
void MULTI_REDUCE::runCudaVariantAtomicGlobal(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      MULTI_REDUCE_INIT_VALUES_RAJA(RAJA::cuda_multi_reduce_atomic);
      ...
      MULTI_REDUCE_FINALIZE_VALUES_RAJA(RAJA::cuda_multi_reduce_atomic);
        ...
        seq_for(gpu_atomic_replications_type{}, [&](auto replication) {
          ...
          if (run_params.numValidAtomicReplication() == 0u ||
            ...
            if (tune_idx == t) {
              ...
              runCudaVariantAtomicGlobal<decltype(block_size)::value, replication>(vid);
