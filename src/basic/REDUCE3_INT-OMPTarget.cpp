...
void REDUCE3_INT::runOpenMPTargetVariant(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx))
{
  ...
  if ( vid == Base_OpenMPTarget ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      RAJA::ReduceSum<RAJA::omp_target_reduce, Int_type> vsum(vsum_init);
      RAJA::ReduceMin<RAJA::omp_target_reduce, Int_type> vmin(vmin_init);
      RAJA::ReduceMax<RAJA::omp_target_reduce, Int_type> vmax(vmax_init);
