...
void INIT_VIEW1D_OFFSET::runHipVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      hipLaunchKernelGGL((initview1d_offset<block_size>), dim3(grid_size), dim3(block_size), 0, res.get_stream(),
          a, v, ibegin, iend );
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      hipLaunchKernelGGL((lambda_hip_forall<block_size, decltype(initview1d_offset_lambda)>),
        grid_size, block_size, 0, res.get_stream(), ibegin, iend, initview1d_offset_lambda);
      hipErrchk( hipGetLastError() );
