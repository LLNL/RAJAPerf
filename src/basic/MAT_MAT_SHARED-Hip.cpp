...
void MAT_MAT_SHARED::runHipVariantImpl(VariantID vid)
{
  ...
  if (vid == Base_HIP) {
    ...
    using launch_policy = RAJA::expt::LaunchPolicy<RAJA::expt::seq_launch_t
                                                   ,RAJA::expt::hip_launch_t<true, tile_size*tile_size>
                                                   >;
...
void MAT_MAT_SHARED::runHipVariant(VariantID vid, size_t tune_idx)
{
  size_t t = 0;
...
void MAT_MAT_SHARED::setHipTuningDefinitions(VariantID vid)
{
  seq_for(gpu_block_sizes_type{}, [&](auto block_size) {
    if (run_params.numValidGPUBlockSize() == 0u ||
        run_params.validGPUBlockSize(block_size)) {
