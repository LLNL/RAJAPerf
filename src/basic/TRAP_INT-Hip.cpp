...
template < size_t block_size >
void TRAP_INT::runHipVariantReduceAtomic(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      RAJA::ReduceSum<RAJA::hip_reduce_atomic, Real_type> sumx(sumx_init);
...
template < size_t block_size >
void TRAP_INT::runHipVariantReduceUnsafeAtomic(VariantID vid)
{
  const Index_type run_reps = getRunReps();
  const Index_type iend = getActualProblemSize();
...
template < size_t block_size >
void TRAP_INT::runHipVariantReduce(VariantID vid)
{
  const Index_type run_reps = getRunReps();
  const Index_type ibegin = 0;
  const Index_type iend = getActualProblemSize();
