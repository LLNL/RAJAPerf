...
void MULADDSUB::runHipVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      hipLaunchKernelGGL((muladdsub<block_size>), dim3(grid_size), dim3(block_size), 0, res.get_stream(),
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      hipLaunchKernelGGL((lambda_hip_forall<block_size, decltype(muladdsub_lambda)>),
        grid_size, block_size, 0, res.get_stream(), ibegin, iend, muladdsub_lambda );
      hipErrchk( hipGetLastError() );
