...
#define SOLVE_4x4_4GROUPS_DATA_SETUP_CUDA          \
  Real_ptr d_a,d_x,d_y;                            \
  allocAndInitCudaDeviceData(d_a, m_a, iend*16*4); \
  allocAndInitCudaDeviceData(d_x, m_x, iend* 4*4); \
  allocAndInitCudaDeviceData(d_y, m_y, iend* 4*4);


#if 0
#endif

#define SOLVE_4x4_4GROUPS_DATA_TEARDOWN_CUDA      \
  getCudaDeviceData(m_y, d_y, iend* 4*4);         \
...
#define SOLVE_4x4_4GROUPS_BODY_CUDA                          \
      Real_type * RAJA_RESTRICT  A = d_a + 16*4 * i;\
      Real_type * RAJA_RESTRICT  x = d_x +  4*4 * i;\
      Real_type * RAJA_RESTRICT  y = d_y +  4*4 * i;\
\
...
__global__ void solve_4x4_4g(Real_type* RAJA_RESTRICT  d_a, Real_type* RAJA_RESTRICT  d_x, Real_type* RAJA_RESTRICT  d_y, Index_type iend)
{
   Index_type i = blockIdx.x * block_size + threadIdx.x;
   if (i < iend) {
       Real_ptr a = d_a+16*4*i;
       Real_ptr x = d_x+ 4*4*i;
       Real_ptr y = d_y+ 4*4*i;
...
template < size_t block_size >
void SOLVE_4x4_4GROUPS::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    SOLVE_4x4_4GROUPS_DATA_SETUP_CUDA;

    startTimer();
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {

      const size_t grid_size = RAJA_DIVIDE_CEILING_INT(iend, block_size);
      solve_4x4_4g<block_size><<<grid_size, block_size>>>( d_a, d_x, d_y, iend );
      cudaErrchk( cudaGetLastError() );

    }
    stopTimer();

    SOLVE_4x4_4GROUPS_DATA_TEARDOWN_CUDA;
    ...
    SOLVE_4x4_4GROUPS_DATA_TEARDOWN_CUDA;
     ...
     getCout() << "\n  SOLVE_4x4_4GROUPS : Unknown Cuda variant id = " << vid << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BIOLERPLATE(SOLVE_4x4_4GROUPS, Cuda)
