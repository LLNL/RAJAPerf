...
TRIDIAGONAL_PAR::TRIDIAGONAL_PAR(const RunParams &params)
    : KernelBase(rajaperf::Basic_TRIDIAGONAL_PAR, params)
{
...
TRIDIAGONAL_PAR::~TRIDIAGONAL_PAR() {}

void TRIDIAGONAL_PAR::setUp(VariantID vid, size_t /*tune_idx*/) {
...
void TRIDIAGONAL_PAR::updateChecksum(VariantID vid, size_t tune_idx) {
  ...
  for (Index_type i = ibegin; i < iend; ++i) {
    ...
    Index_type idx_1 = TRIDIAGONAL_PAR_INDEX(1);
    ...
    Index_type idx_M = TRIDIAGONAL_PAR_INDEX(N-2);
    Index_type idx_N = TRIDIAGONAL_PAR_INDEX(N-1);
    Real_type bN = Aa[idx_N] * x[idx_M] +
                   Ab[idx_N] * x[idx_N];
    checksum[vid][tune_idx] += std::abs(bN - b[idx_N]) * checksum_scale_factor;
  }
...
void TRIDIAGONAL_PAR::tearDown(VariantID vid, size_t /*tune_idx*/) {
  (void)vid;
