...
\
  Real_ptr coeff; \
\
  allocAndInitOpenMPDeviceData(in, m_in, getRunProblemSize(), did, hid); \
  allocAndInitOpenMPDeviceData(out, m_out, getRunProblemSize(), did, hid); \
...
#define FIR_DATA_TEARDOWN_OMP_TARGET \
  getOpenMPDeviceData(m_out, out, getRunProblemSize(), hid, did); \
