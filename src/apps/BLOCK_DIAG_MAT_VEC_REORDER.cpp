...
#include "BLOCK_DIAG_MAT_VEC_REORDER_REORDER.hpp"
...
BLOCK_DIAG_MAT_VEC_REORDER::BLOCK_DIAG_MAT_VEC_REORDER(const RunParams &params)
    : KernelBase(rajaperf::Apps_BLOCK_DIAG_MAT_VEC_REORDER, params)
{
  m_N_default = 1000;
...
BLOCK_DIAG_MAT_VEC_REORDER::~BLOCK_DIAG_MAT_VEC_REORDER() {}

void BLOCK_DIAG_MAT_VEC_REORDER::setUp(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx)) {
...
void BLOCK_DIAG_MAT_VEC_REORDER::updateChecksum(VariantID vid, size_t tune_idx) {
  checksum[vid][tune_idx] += calcChecksum(m_Y, m_ndof*m_NE, checksum_scale_factor );
}

void BLOCK_DIAG_MAT_VEC_REORDER::tearDown(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx)) {
  (void)vid;
  deallocData(m_Me);
  deallocData(m_X);
  deallocData(m_Y);
