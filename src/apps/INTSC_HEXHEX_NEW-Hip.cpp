...
template < Size_type block_size >
void INTSC_HEXHEX_NEW::runHipVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      auto intsc_hexhex_lambda = [=] __device__ ( Index_type i ) {
          ...
          __shared__ Real_type vv_reduce[hexhex_new_len_vv_reduce] ;  // hexhex_new_len_vv_reduce=16
    ...
    stopTimer();
     ...
     getCout() << "\n  INTSC_HEXHEX_NEW : Unknown Hip variant id = " << vid << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(INTSC_HEXHEX_NEW, Hip, Base_HIP, Lambda_HIP, RAJA_HIP)
