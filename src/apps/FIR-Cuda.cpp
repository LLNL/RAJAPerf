...
#define FIR_DATA_SETUP_CUDA \
  allocAndInitCudaDeviceData(in, m_in, getRunProblemSize()); \
  allocAndInitCudaDeviceData(out, m_out, getRunProblemSize()); \
  cudaMemcpyToSymbol(coeff, coeff_array, FIR_COEFFLEN * sizeof(Real_type));


#define FIR_DATA_TEARDOWN_CUDA \
  getCudaDeviceData(m_out, out, getRunProblemSize()); \
