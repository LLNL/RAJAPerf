...
void NODAL_ACCUMULATION_3D::setUp(VariantID vid, size_t /*tid*/)
{
  allocAndInitDataConst(m_x, m_nodal_array_length, 0.0, vid);
  allocAndInitDataConst(m_vol, m_zonal_array_length, 1.0, vid);
}

void NODAL_ACCUMULATION_3D::updateChecksum(VariantID vid, size_t tid)
{
  checksum[vid].at(tid) += calcChecksum(m_x, m_nodal_array_length, checksum_scale_factor );
}

void NODAL_ACCUMULATION_3D::tearDown(VariantID vid, size_t /*tid*/)
{
  (void) vid;
