...
template < size_t block_size >
void MATVEC_3D::runHipVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    stopTimer();
     ...
     getCout() << "\n  MATVEC_3D : Unknown Hip variant id = " << vid << std::endl;
  }
}

RAJAPERF_GPU_BLOCK_SIZE_TUNING_DEFINE_BOILERPLATE(MATVEC_3D, Hip)
