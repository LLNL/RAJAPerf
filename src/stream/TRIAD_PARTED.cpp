...
TRIAD_PARTED::TRIAD_PARTED(const RunParams& params)
  : KernelBase(rajaperf::Stream_TRIAD_PARTED, params)
{
...
TRIAD_PARTED::~TRIAD_PARTED()
{
}

void TRIAD_PARTED::setUp(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx))
{
...
void TRIAD_PARTED::updateChecksum(VariantID vid, size_t tune_idx)
{
  checksum[vid][tune_idx] += calcChecksum(m_a, getActualProblemSize(), checksum_scale_factor , vid);
}

void TRIAD_PARTED::tearDown(VariantID vid, size_t RAJAPERF_UNUSED_ARG(tune_idx))
{
  (void) vid;
  deallocData(m_a, vid);
  deallocData(m_b, vid);
